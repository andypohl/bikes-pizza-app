...
void main() {
  ...
  test(
    'posts to the versioned GraphQL endpoint with the token header',
    () async {
  ...
  test('parses products, prices, images and paging', () async {
    final client = MockClient(
      (_) async => http.Response(_productsBody(hasNext: true), 200),
    );
  ...
  test(
    'createCheckout returns the checkout URL and passes the email',
    () async {
      late Map<String, dynamic> variables;
      final client = MockClient((req) async {
        variables =
            (jsonDecode(req.body) as Map)['variables'] as Map<String, dynamic>;
      ...
      expect((input['lines'] as List).single, {
        'merchandiseId': 'gid://shopify/ProductVariant/a-1',
        'quantity': 1,
      });
