...
**Today.** The website is static: Astro reads the posts from Firestore
on the GitHub Actions runner and writes plain HTML, which Firebase Hosting
serves from its CDN. Content changes reach the site through a rebuild
(the functions trigger the "Rebuild website" workflow after publishing or
changing a post). Parts of a page that must be fresh are "islands" that
query Firebase from the browser, search being the first.

**The idea.** Render pages on request instead, on Cloudflare Workers
(Astro's Cloudflare adapter), querying Firestore each time. Cloudflare's
cache sits in front of the Worker, so traffic spikes are still absorbed;
the functions purge the cache rather than rebuilding. Every page is then
always current, with no rebuild step.
